...
NSString * const HOST = @"https://pay.hutko.org";
...
@interface PSHutkoApi () <NSURLSessionDelegate, PKPaymentAuthorizationViewControllerDelegate>
...
@property (nonatomic, weak) id<PSHutkoView> HutkoView;

@end

@implementation PSHutkoApi

+ (BOOL)supportsApplePay {
    return [PKPaymentAuthorizationViewController canMakePayments];
}

+ (instancetype)apiWithMerchant:(NSInteger)merchantId andHutkoView:(id<PSHutkoView>)HutkoView;
{
    PSHutkoApi *api = [[PSHutkoApi alloc] init];
    api.merchantId = merchantId;
    api.HutkoView = HutkoView;
    return api;
}
                           ...
                           withCallback:(PSIsPayerEmailRequiredCallback)callback
{
    ...
    [self jsonNetworkRequestByPath:@"/api/checkout/merchant/info"
                          jsonBody:[PSHutkoApi requestJson:params]
...
- (void)isPayerEmailRequiredForToken:(NSString *)token
    ...
    [self jsonNetworkRequestByPath:@"/api/checkout/ajax/mobile_pay"
                          jsonBody:[PSHutkoApi requestJson:@{@"token":token}]
                        onComplete:^(NSDictionary *json, NSError *error) {
        if (error) {
            dispatch_async(dispatch_get_main_queue(), ^{
                callback(NO, error);
...
- (void)getToken:(PSOrder *)order
    ...
    [self payJsonNetworkRequestByPath:@"/api/checkout/token"
                             jsonBody:[PSHutkoApi requestJson:dictionary]
...
- (void)applePayConfig:(NSString *)currency
    ...
    [self payJsonNetworkRequestByPath:@"/api/checkout/ajax/mobile_pay"
        ...
        if (data == nil) {
            [delegate onPaidFailure:[NSError errorWithDomain:@"HutkoApi" code:PSPayErrorCodeApplePayUnsupported userInfo:nil]];
        } else {
...
- (void)url3ds:(PSCheckout *)checkout aPayCallbackDelegate:(id<PSPayCallbackDelegate>)delegate {
    void (^successCallback)(NSData * data) = ^(NSData *data) {
        ...
        [self.HutkoView confirm:confirmation];
    ...
    if (checkout.sendData.paReq == nil) {
        ...
        [self payNetworkRequestByURL:[NSURL URLWithString:checkout.url]
            ...
            onIntercept:^(NSMutableURLRequest *request) {
                NSString *post = [NSString stringWithFormat:@"MD=%@&PaReq=%@&TermUrl=%@", [PSHutkoApi encodeToPercentEscapeString:checkout.sendData.md], [PSHutkoApi encodeToPercentEscapeString:checkout.sendData.paReq], [PSHutkoApi encodeToPercentEscapeString:checkout.sendData.termUrl]];
...
API_AVAILABLE(ios(11.0))
{
    ...
    NSDictionary *paymentMethod = @{
                                    ...
                                    @"type": [PSHutkoApi paymentMethodName: payment.token.paymentMethod.type],
                                    };
...
- (void)jsonNetworkRequestByPath:(NSString *)path
    ...
    [self networkRequest:[NSURL URLWithString:[NSString stringWithFormat: @"%@%@", HOST, path]]
        ...
        [PSHutkoApi setRequestJsonBody:request jsonBody:jsonBody];
    }];
}
...
- (void)networkRequest:(NSURL *)url
    ...
    NSURLSessionDataTask *postDataTask = [session dataTaskWithRequest:request
                                          ...
                                          {
                                              if (error) {
                                                  error = [NSError errorWithDomain:@"HutkoApi" code:PSPayErrorCodeNetworkAccess userInfo:nil];
                                                  complete(nil, error);
                                              } else {
                                                  @try {
                                                      complete(data, nil);
                                                  ...
                                                  @catch (NSException *exception) {
                                                      ...
                                                      if (exception.userInfo == nil) {
                                                          error = [NSError errorWithDomain:@"HutkoApi" code:PSPayErrorCodeUnknown userInfo:nil];
                                                      } else {
                                                          error = [NSError errorWithDomain:@"HutkoApi" code:PSPayErrorCodeFailure userInfo:exception.userInfo];
                                                      }
                                                      complete(nil, error);
